// Серверная функция-прокси для Vercel. Ключ OpenRouter хранится в переменных окружения проекта,
...
module.exports = async (req, res) => {
  ...
  if (!apiKey) {
    res.status(500).json({ error: "OPENROUTER_API_KEY не задан в настройках проекта на Vercel" });
    return;
  }
  ...
  try {
    const { messages, company_id, max_tokens } = req.body || {};
